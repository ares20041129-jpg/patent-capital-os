#!/usr/bin/env python3
"""Validate application-number or acceptance evidence after receipt capture."""

from __future__ import annotations

import argparse
import hashlib
import json
import re
from pathlib import Path
from typing import Any


SHA256_RE = re.compile(r"^sha256:[0-9a-f]{64}$")

REQUIRED_PATHS = [
    "case_id",
    "previous_status",
    "status",
    "official_system",
    "official_session_authorization_hash",
    "official_session_reference_hash",
    "checked_at",
    "checked_by",
    "official_receipt.receipt_id",
    "official_receipt.receipt_file",
    "official_receipt.receipt_hash",
    "application.application_number",
    "application.filing_date",
    "application.official_status",
    "application.official_status_snapshot",
    "application.official_status_snapshot_hash",
    "application.official_file_list_file",
    "application.official_file_list_hash",
    "package.final_package_hash",
    "package.reviewed_package_hash",
    "package.submitted_package_hash",
    "fees.payment_status",
    "docket.docket_entry_id",
    "docket.docket_entry_hash",
    "docket.next_deadlines",
    "portfolio.family_id",
    "portfolio.portfolio_tags",
    "portfolio.maintenance_owner",
    "decision.status",
    "decision.reason",
]

HASH_PATHS = [
    "official_receipt.receipt_hash",
    "official_session_authorization_hash",
    "official_session_reference_hash",
    "application.official_status_snapshot_hash",
    "application.official_file_list_hash",
    "package.final_package_hash",
    "package.reviewed_package_hash",
    "package.submitted_package_hash",
    "docket.docket_entry_hash",
]


def load_json(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def sha256_file(path: Path) -> str:
    return "sha256:" + hashlib.sha256(path.read_bytes()).hexdigest()


def get_path(data: dict[str, Any], dotted: str) -> Any:
    cur: Any = data
    for part in dotted.split("."):
        if not isinstance(cur, dict) or part not in cur:
            return None
        cur = cur[part]
    return cur


def is_blank(value: Any) -> bool:
    return value is None or value == "" or value == [] or value == {}


def require_hash(data: dict[str, Any], dotted: str, errors: list[str]) -> None:
    value = get_path(data, dotted)
    if is_blank(value):
        errors.append(f"missing_required_field: {dotted}")
    elif not SHA256_RE.fullmatch(str(value)):
        errors.append(f"{dotted}_must_be_sha256_64_hex")


def resolve_reference(reference: Any, base_dir: Path) -> Path:
    path = Path(str(reference or ""))
    return path if path.is_absolute() else (base_dir / path).resolve()


def validate_file_hash_reference(
    data: dict[str, Any],
    path_key: str,
    hash_key: str,
    label: str,
    base_dir: Path | None,
    errors: list[str],
) -> None:
    if base_dir is None or is_blank(get_path(data, path_key)):
        return
    path = resolve_reference(get_path(data, path_key), base_dir)
    if not path.exists():
        errors.append(f"{label}_file_not_found")
        return
    if get_path(data, hash_key) and sha256_file(path) != get_path(data, hash_key):
        errors.append(f"{label}_hash_mismatch")


def validate(data: dict[str, Any], base_dir: Path | None = None) -> tuple[bool, list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []

    for path in REQUIRED_PATHS:
        if is_blank(get_path(data, path)):
            errors.append(f"missing_required_field: {path}")

    for path in HASH_PATHS:
        require_hash(data, path, errors)

    for path_key, hash_key, label in [
        ("official_receipt.receipt_file", "official_receipt.receipt_hash", "official_receipt"),
        ("application.official_status_snapshot", "application.official_status_snapshot_hash", "official_status_snapshot"),
        ("application.official_file_list_file", "application.official_file_list_hash", "official_file_list"),
    ]:
        validate_file_hash_reference(data, path_key, hash_key, label, base_dir, errors)

    if data.get("previous_status") != "official_receipt_received":
        errors.append("previous_status_must_be_official_receipt_received")

    if data.get("status") != "accepted_or_application_number_received":
        errors.append("status_must_be_accepted_or_application_number_received")

    if get_path(data, "decision.status") != "accepted_or_application_number_received":
        errors.append("decision_status_must_be_accepted_or_application_number_received")

    final_hash = get_path(data, "package.final_package_hash")
    reviewed_hash = get_path(data, "package.reviewed_package_hash")
    submitted_hash = get_path(data, "package.submitted_package_hash")
    if final_hash and reviewed_hash and final_hash != reviewed_hash:
        errors.append("hash_mismatch: package.final_package_hash != package.reviewed_package_hash")
    if final_hash and submitted_hash and final_hash != submitted_hash:
        errors.append("hash_mismatch: package.submitted_package_hash != package.final_package_hash")

    app_number = str(get_path(data, "application.application_number") or "")
    if len(app_number.strip()) < 6:
        errors.append("application_number_too_short")

    official_status = str(get_path(data, "application.official_status") or "").lower()
    if official_status not in {
        "accepted",
        "application_number_received",
        "filed",
        "受理",
        "已受理",
        "申请号已取得",
    }:
        errors.append("official_status_not_acceptance_or_application_number")

    payment_status = str(get_path(data, "fees.payment_status") or "").lower()
    if payment_status not in {"paid", "pending", "not_due", "deferred"}:
        errors.append("fees.payment_status_invalid")
    if payment_status == "paid":
        require_hash(data, "fees.payment_receipt_hash", errors)
        validate_file_hash_reference(data, "fees.payment_receipt_file", "fees.payment_receipt_hash", "payment_receipt", base_dir, errors)

    tags = get_path(data, "portfolio.portfolio_tags")
    if not isinstance(tags, list) or not tags:
        errors.append("portfolio_tags_must_be_nonempty_list")

    deadlines = get_path(data, "docket.next_deadlines")
    if not isinstance(deadlines, list) or not deadlines:
        errors.append("docket_next_deadlines_must_be_nonempty_list")

    if data.get("benchmark_mock") is True:
        warnings.append("benchmark_mock_not_real_application_number")

    return len(errors) == 0, errors, warnings


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("evidence", type=Path)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()

    try:
        ok, errors, warnings = validate(load_json(args.evidence), base_dir=args.evidence.parent)
    except Exception as exc:
        ok, errors, warnings = False, [str(exc)], []

    result = {"ok": ok, "errors": errors, "warnings": warnings}
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2))
    else:
        print("PASS" if ok else "FAIL")
        for error in errors:
            print(f"ERROR: {error}")
        for warning in warnings:
            print(f"WARNING: {warning}")
    return 0 if ok else 1


if __name__ == "__main__":
    raise SystemExit(main())
